...
/**
 * V3 Print View
 * Prints Guarantee Letters (Extension/Release)
 */
...
// Check history_id param
$historyId = $_GET['history_id'] ?? null;

if (!$guaranteeId && !$snapshotParam && !$historyId) {
...
if ($historyId) {
    ...
    $hist = $historyRepo->find((int)$historyId);
    if ($hist && !empty($hist['snapshot_data'])) {
        $snapshot = $hist['snapshot_data'];
        $eventDate = $hist['created_at'];
        $isHistorical = true;
    }
}
...
if ($isHistorical && $snapshot) {
    ...
    $data->guaranteeNumber = $snapshot['guarantee_number'] ?? '';
    $data->contractNumber = $snapshot['contract_number'] ?? '';
    // Handle amount format (might be string with commas in snapshot)
    $amt = str_replace(',', '', $snapshot['amount'] ?? '0');
    $data->amount = (float) $amt;
    $data->expiryDate = $snapshot['expiry_date'] ?? null;
    $data->type = $snapshot['type'] ?? '';
    $data->supplierName = $snapshot['supplier_name'] ?? 'غير محدد';
    $data->bankName = $snapshot['bank_name'] ?? 'غير محدد';
    $data->isRelease = ($snapshot['action_type'] ?? '') === 'release';
    
    // Historical mode lacks bank details unless we fetch by name match, 
    // but usually historical view relies purely on snapshot. 
    // We'll leave bank details default.
    ...
    // 1. Try Decision (Supplied ID)
    // 2. Try Raw Import
    $supplierName = $raw['supplier'] ?? 'غير محدد';
    // Ideally we should get the decision to find the real supplier
    // For V3 phase 1, we might rely on raw data or fetch decision.
    // Let's check if we have a decision
    ...
    if ($decision && $decision['supplier_id']) {
        ...
        if ($supplier) {
            $supplierName = $supplier->officialName;
        }
    }
    ...
    if ($decision && $decision['bank_id']) {
        ...
        if ($bank) {
            $bankName = $bank->officialName;
            $data->bankDept = $bank->department ?? $data->bankDept;
            $data->bankAddress = array_filter([
                $bank->addressLine1,
                $bank->addressLine2
            ]) ?: $data->bankAddress;
    ...
    $data->guaranteeNumber = $guarantee->guaranteeNumber;
    $data->contractNumber = $raw['contract_number'] ?? '';
    $data->amount = $raw['amount'] ?? 0.0;
    $data->expiryDate = $raw['expiry_date'] ?? null;
    $data->type = $raw['type'] ?? '';
    $data->supplierName = $supplierName;
    $data->bankName = $bankName;
    
    // Check Action Type from GET or default to Extension
    $actionType = $_GET['action'] ?? 'extension'; // extension, release
    $data->isRelease = ($actionType === 'release');
}
...
if ($data->type) {
    $t = mb_strtoupper($data->type); // mb_ for unicode safety
    // Mapping Logic (can be refined)
...
<head>
    <meta charset="UTF-8">
    <title>طباعة خطاب - <?= htmlspecialchars($data->guaranteeNumber) ?></title>
    <!-- Use CDN for Tailwind to ensure it works without build step -->
    ...
    <style>
        ...
        @media print {
            ...
            .letter-paper { box-shadow: none; border: none; margin: 0; padding: 20mm; width: 100% !important; height: auto !important; }
        }
    </style>
</head>
<body>
    <?php if ($isHistorical): ?>
    <div class="no-print fixed top-0 left-0 right-0 bg-amber-600 text-white p-3 z-50 flex items-center justify-center gap-3 shadow-lg">
        <span class="text-2xl">⚠️</span>
        <div>
            <div class="text-lg font-bold">عرض تاريخي - هذا هو الخطاب كما كان في تاريخ الحدث</div>
            <?php if ($eventDate): ?>
            <div class="text-sm opacity-90">التاريخ: <?= htmlspecialchars($eventDate) ?></div>
            <?php endif; ?>
        </div>
    </div>
    <?php endif; ?>
    
    <div class="no-print fixed top-20 right-5 z-40 flex flex-col gap-2">

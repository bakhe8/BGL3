<section id="proposals" class="glass-card">
    ...
    <?php
        $hasProposals = !empty($proposals);
        $automationMode = (isset($approvalsEnabled, $autoApplyEnabled) && !$approvalsEnabled && $autoApplyEnabled);
    ?>
    <?php if ($automationMode): ?>
        <p style="color: var(--text-secondary); font-style: italic;">
            الاقتراحات مؤتمتة حالياً؛ لا حاجة لإجراءات يدوية.
        </p>
        <?php if ($hasProposals): ?>
            <div style="color: var(--text-secondary); font-size:0.85rem; margin-top:6px;">
                عدد الاقتراحات الحالية: <?= count($proposals) ?> (تُنفّذ تلقائياً).
            </div>
            <ul style="list-style:none; padding:0; margin:8px 0 0;">
                <?php foreach($proposals as $p): ?>
                    <li style="padding:8px 0; border-bottom:1px solid var(--glass-border);" data-item="proposal">
                        <strong style="color: var(--accent-cyan);"><?= htmlspecialchars($p['name'] ?? $p['id'] ?? 'اقتراح') ?></strong>
                        <?php if (!empty($p['status'])): ?>
                            <span style="color: var(--text-secondary); font-size:0.8rem;">
                                | الحالة: <?= htmlspecialchars($p['status']) ?>
                            </span>
                        <?php endif; ?>
                        <?php if (!empty($p['status_note'])): ?>
                            <div style="margin-top:4px; font-style:italic; color:var(--text-secondary); font-size:0.8rem;">
        ...
        <?php else: ?>
            <p data-empty="proposal" style="color: var(--text-secondary); font-style: italic;">لا توجد اقتراحات حالياً.</p>
        <?php endif; ?>
    <?php else: ?>

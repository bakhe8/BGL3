                    ...
                    <div class="timeline-event-wrapper" 
                        ...
                        <div class="timeline-event-card" style="background: white; border: 1px solid #e2e8f0; border-right: 3px solid <?= $colors['border'] ?>; border-radius: 4px; padding: 10px 12px; margin-right: 16px; transition: all 0.2s;" 
                            ...
                            <div style="font-size: 12px; color: #475569; line-height: 1.6; margin: 6px 0; padding: 6px 8px; background: #f8fafc; border-radius: 3px;">
                                ...
                                // Pattern 1: "تغيير X من [old] إلى [new]"
                                ...
                                // Pattern 2: AI match "مطابقة تلقائية لل...: ... -> ... (...%)"
                                if (!$parsed && preg_match('/مطابقة تلقائية (لل.*?): (.*?) -?> (.*?) \((\d+)%\)/', $reason, $match)) {
                                    $field = trim($match[1]);
                                    $oldVal = trim($match[2]);
                                    $newVal = trim($match[3]);
                                ...
                                // Pattern 3: "تخفيض المبلغ من [X] إلى [Y]"
                                if (!$parsed && preg_match('/تخفيض المبلغ من \[(.*?)\] إلى \[(.*?)\]/', $reason, $match)) {
                                    $oldVal = trim($match[1]);
                                    $newVal = trim($match[2]);
                                    ?>
                                    <div style="margin-bottom: 4px;">
                                        <strong style="color: #1e293b;">• المبلغ:</strong>
                                        <span style="color: #dc2626; text-decoration: line-through; opacity: 0.8;">
                                            <?= htmlspecialchars($oldVal) ?>
                                        </span>
                                        <span style="color: #64748b; margin: 0 4px;">→</span>
                                ...
                                // Pattern 4: "تم تمديد الضمان لمدة سنة" or similar
                                if (!$parsed && (strpos($reason, 'تمديد') !== false || strpos($reason, 'ممدد') !== false)) {
                                    ?>
                                ...
                                // Pattern 5: "الإفراج عن الضمان" or releases
                                if (!$parsed && (strpos($reason, 'إفراج') !== false || strpos($reason, 'الإفراج') !== false)) {
                                    ?>
                                    <div style="color: #059669; font-weight: 500;">
                                        ✓ <?= htmlspecialchars($reason) ?>
                                    </div>
                                ...
                                // Pattern 6: "تم ربط المورد يدوياً: [name]"
                                if (!$parsed && preg_match('/تم ربط (.*?) يدوياً: \[(.*?)\]/', $reason, $match)) {
                                    $field = trim($match[1]);
                                    $value = trim($match[2]);
                                    ?>
                                    <div style="margin-bottom: 4px;">
                                        ...
                                        <span style="color: #64748b; font-size: 11px; margin-left: 4px;">(ربط يدوي)</span>
                                    </div>
                                    <?php
                                    $parsed = true;
                                }
                                
                                // Fallback: Display as-is if no pattern matched
                                if (!$parsed) {

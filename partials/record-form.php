...
<header class="card-header">
    <div class="header-title">
        <h2>تفاصيل الضمان</h2>
        <span class="badge badge-<?= $record['status'] === 'approved' ? 'success' : 'warning' ?>">
        ...
        <div class="chips-row" id="supplier-suggestions" <?= $isHistorical ? 'style="display:none"' : '' ?>>
            <?php if (!empty($supplierMatch['suggestions'])): ?>
                <?php foreach ($supplierMatch['suggestions'] as $sugg): ?>
                    <button class="chip <?= ($record['supplier_name'] === $sugg['official_name']) ? 'chip-selected' : 'chip-candidate' ?>" 
                            data-action="selectSupplier"
                            data-id="<?= $sugg['id'] ?>"
                            data-name="<?= htmlspecialchars($sugg['official_name']) ?>">
                        <?php if (($sugg['score'] ?? 0) > 90): ?><span>⭐ </span><?php endif; ?>
                        <span><?= htmlspecialchars($sugg['official_name']) ?></span>
                        <span class="chip-source"><?= $sugg['score'] ?>%</span>
                    </button>
                <?php endforeach; ?>
            <?php else: ?>

...
commands:
  ...
  invalid:                # (اختياري) التعامل مع JSON غير الصالح
...
features:
  # ... بقية الإعدادات ...
  aggregate_include_debounced: false   # (اختياري) تضمين debounced_skipped في أحداث aggregate
...
- عند فشل قراءة JSON (مثلاً الملف يُكتب حالياً)، يمكن ضبط السلوك عبر commands.invalid:
  - max_retries: عدد المحاولات قبل اعتبار الملف غير صالح (الافتراضي 3)
  - quarantine_enabled: إن كانت true يُنقل الملف إلى invalid_dir ويُكتب <stem>.error.json، وإن كانت false يبقى في inbox للمحاولة لاحقاً
  - invalid_dir: مسار مجلد العزل (افتراضياً agent/commands/inbox/invalid)

ترقية الإصدار
- تم تحديث نسخة الوكيل إلى 1.3.2-stage4.

...
try:
    ...
    sys.stderr.write("watchdog غير مثبت. قم بتثبيته أولاً:\n  pip install watchdog\n")
    sys.exit(1)
...
class RuntimeConfig:
    ...
    # invalid JSON handling (commands)
    invalid_quarantine_enabled: bool
    invalid_max_retries: int
    invalid_dir: Path
    # behavior
    debounce_ms: float
...
def load_config(agent_dir: Path, project_root: Path) -> RuntimeConfig:
    ...
    defaults: Dict[str, Any] = {
        ...
        "commands": {
            ...
            "invalid": {
                ...
                "max_retries": 3,
                "invalid_dir": str(agent_dir / "commands" / "inbox" / "invalid"),
            },
        },
    }
    ...
    if cfg_path.exists() and yaml is not None:
        try:
            ...
            sys.stderr.write(
                f"Failed to read config.yml, using defaults. Error: {exc}\n"
            )
    elif cfg_path.exists() and yaml is None:
        sys.stderr.write(
            "PyYAML غير مثبت. تجاهل config.yml واستخدام الإعدادات الافتراضية.\n"
    ...
    # Pre-resolve command paths for dependent defaults
    inbox_dir = Path(
        data.get("commands", {}).get("inbox", str(agent_dir / "commands" / "inbox"))
    ).resolve()
    outbox_dir = Path(
        data.get("commands", {}).get("outbox", str(agent_dir / "commands" / "outbox"))
    ).resolve()
    invalid_cfg = data.get("commands", {}).get("invalid", {}) or {}
    invalid_quarantine_enabled = bool(invalid_cfg.get("quarantine_enabled", True))
    invalid_max_retries = int(invalid_cfg.get("max_retries", 3))
    invalid_dir = Path(
        invalid_cfg.get("invalid_dir", str(inbox_dir / "invalid"))
    ).resolve()

    behavior = data.get("behavior", {})
    if not behavior:
        # fallback to features for backward compat
        behavior = features
    ...
    return RuntimeConfig(
        ...
        event_types=set(
            map(str, features.get("event_types", ["created", "modified", "deleted"]))
        ),
        log_path=log_path,
        ...
        command_poll_interval=float(
            data.get("commands", {}).get("poll_interval_ms", 1000)
        )
        / 1000.0,
        invalid_quarantine_enabled=invalid_quarantine_enabled,
        invalid_max_retries=invalid_max_retries,
        invalid_dir=invalid_dir,
        debounce_ms=float(behavior.get("debounce_ms", 150)),
        aggregate_window_ms=float(behavior.get("aggregate_window_ms", 1000)),
        aggregate_include_debounced=bool(
            behavior.get("aggregate_include_debounced", False)
        ),
    )
...
class ProjectEventHandler(FileSystemEventHandler):
    ...
    def _normalized(self, p: bytes | str | Path) -> Path:
    ...
    def _is_ignored(self, path: bytes | str | Path) -> bool:
        try:
    ...
    def _rel(self, p: bytes | str | Path) -> str:
        try:
    ...
    def _emit(self, event: FileSystemEvent, change: str) -> None:
        ...
        try:
            ...
            if self.state.config:
                try:
                    debounce_ms = float(
                        getattr(self.state.config, "debounce_ms", 0.0) or 0.0
                    )
                except Exception:
                    debounce_ms = 0.0
            if debounce_ms > 0:
            ...
            self.logger.exception(
                "Error while handling event %s for %s: %s", change, event.src_path, exc
            )

    # Only handle the required three types explicitly
    def on_created(self, event: FileSystemEvent) -> None:
        if not event.is_directory and (
            not self.state.config or "created" in self.state.config.event_types
        ):
            self._emit(event, "created")

    def on_modified(self, event: FileSystemEvent) -> None:
        if not event.is_directory and (
            not self.state.config or "modified" in self.state.config.event_types
        ):
            self._emit(event, "modified")

    def on_deleted(self, event: FileSystemEvent) -> None:
        if not event.is_directory and (
            not self.state.config or "deleted" in self.state.config.event_types
        ):
...
def main() -> int:
    try:
        ...
        logger.info(
            "Agent starting. Watching: %s (recursive=%s)", watch_path, state.recursive
        )
        if cfg.feature_console_log:
        ...
        t_status = threading.Thread(
            target=status_worker, name="agent-status", daemon=True
        )
        t_status.start()
        t_config = threading.Thread(
            target=config_worker, name="agent-config", daemon=True
        )
        t_config.start()
        ...
        def aggregate_worker() -> None:
            while not stop_event.is_set():
                try:
                    ...
                    try:
                        window_ms = float(
                            getattr(cfg_local, "aggregate_window_ms", 0.0) or 0.0
                        )
                    except Exception:
                        window_ms = 0.0
                    if window_ms <= 0:
                    ...
                    try:
                        include_debounced = bool(
                            getattr(cfg_local, "aggregate_include_debounced", False)
                        )
                    except Exception:
                        include_debounced = False
                    with state._agg_lock:
                        counts = dict(state._agg_counts)
                        debounced_skipped = int(
                            getattr(state, "_agg_debounced_skipped", 0)
                        )
                    ...
                    if sum(counts.values()) > 0:
                        try:
                            cfg_local.jsonl_path.parent.mkdir(
                            ...
                            logger.exception(
                                "Failed to append aggregate JSONL: %s", exc
                            )
                except Exception as exc:
        ...
        def command_worker() -> None:
            while not stop_event.is_set():
                ...
                try:
                    ...
                    invalid_dir = cfg_local.invalid_dir
                    processed_dir.mkdir(parents=True, exist_ok=True)
                    # Create invalid dir only if feature enabled
                    if cfg_local.invalid_quarantine_enabled:
                    ...
                    for cmd_file in inbox.glob("*.json"):
                        try:
                            ...
                            if (time.time() - st.st_mtime) < 0.05:
                                continue
                            # Accept UTF-8 with BOM from external writers (e.g., PowerShell)
                            with cmd_file.open("r", encoding="utf-8-sig") as f:
                                data = json.load(f)
                            ...
                            if op == "pause":
                                ...
                                resp["ignored_paths"] = [
                                    str(p) for p in sorted(new_extra_paths)
                                ]
                                ...
                                resp["ignored_paths"] = [
                                    str(p) for p in sorted(state.extra_ignored_paths)
                                ]
                                ...
                                resp["base_ignored_paths"] = [
                                    str(p) for p in sorted(state.ignored_paths)
                                ]
                                resp["base_ignored_globs"] = sorted(state.ignore_globs)
                                resp["extra_ignored_paths"] = [
                                    str(p) for p in sorted(state.extra_ignored_paths)
                                ]
                                resp["extra_ignored_globs"] = sorted(
                                    state.extra_ignore_globs
                                )
                                msg = "ignored rules returned"
                            elif op == "rotate_logs":
                                try:
                                    with state.logging_lock:
                                        ...
                                        ts_suffix = datetime.now().strftime(
                                            "%Y%m%d-%H%M%S"
                                        )
                                        rotated = {}
                                        errors = []

                                        # Individual rotation for log
                                        if cfg_local.log_path.exists():
                                            rotated_log = cfg_local.log_path.with_name(
                                                cfg_local.log_path.stem
                                                + f".{ts_suffix}"
                                                + cfg_local.log_path.suffix
                                            )
                                            try:
                                                cfg_local.log_path.rename(rotated_log)
                                                rotated["log"] = str(rotated_log)
                                            except Exception as e:
                                                errors.append(f"log rotate failed: {e}")

                                        # Individual rotation for jsonl
                                        if cfg_local.jsonl_path.exists():
                                            rotated_jsonl = (
                                                cfg_local.jsonl_path.with_name(
                                                    cfg_local.jsonl_path.stem
                                                    + f".{ts_suffix}"
                                                    + cfg_local.jsonl_path.suffix
                                                )
                                            )
                                            try:
                                                cfg_local.jsonl_path.rename(
                                                    rotated_jsonl
                                                )
                                                rotated["jsonl"] = str(rotated_jsonl)
                                            except Exception as e:
                                                errors.append(
                                                    f"jsonl rotate failed: {e}"
                                                )

                                        # Reconfigure anyway to ensure we are logging to new/reset files
                            ...
                            # Retry and after threshold optionally move to invalid with error report
                            try:
                                key = cmd_file.name
                                cnt = state.command_retries.get(key, 0) + 1
                                state.command_retries[key] = cnt
                                if cfg_local.invalid_quarantine_enabled and cnt >= int(
                                    getattr(cfg_local, "invalid_max_retries", 3)
                                ):
                                    err = {
                                        "ts": datetime.now(timezone.utc).isoformat(),
                                    ...
                                    err_path = (
                                        cfg_local.invalid_dir
                                        / f"{cmd_file.stem}.error.json"
                                    )
                                    write_json_atomic(err_path, err)
                                    dest = cfg_local.invalid_dir / cmd_file.name
                                    try:
                                        cmd_file.replace(dest)
                                    except Exception:
                                        try:
                            ...
                            logger.exception(
                                "Failed processing command %s: %s", cmd_file, exc
                            )
        ...
        t_agg = threading.Thread(
            target=aggregate_worker, name="agent-aggregate", daemon=True
        )
        t_agg.start()
        t_cmd = threading.Thread(
            target=command_worker, name="agent-commands", daemon=True
        )
        t_cmd.start()
        ...
        sys.stderr.write(f"[{ts}] Fatal agent error: {exc}\n")
        return 1

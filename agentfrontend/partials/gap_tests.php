<div class="glass-card">
    <div class="card-header">Gap Tests (مختصر)</div>
    <ul style="list-style: none; padding: 0; margin: 0;">
        <li style="padding: 8px 0; border-bottom: 1px solid var(--glass-border);">
            ...
            <span style="color: <?= $gapRateLimit['passed'] ? 'var(--success)' : 'var(--danger)' ?>; font-weight:700; margin-right:10px;">
                <?= $gapRateLimit['passed'] ? 'PASS' : 'FAIL' ?>
            </span>
            <?php if(!$gapRateLimit['passed']): ?>
                <div style="color: var(--text-secondary); font-size: 0.85rem;">لا توجد دلائل على RateLimit في autoload/routes.</div>
            <?php else: ?>
